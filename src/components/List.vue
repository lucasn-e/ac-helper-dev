<template>
    <div class="table-container">
        <table class="data-table">
            ...
            <tbody v-if="payload.sort !== 'month'">
                <tr v-for="data in sortedData" :key="`data-${data.name}`" :class="checkCaught(data) ? 'content-row caught' : 'content-row'" @click="setCaught(data)">
                    <td class="caught">
                            <div :class="checkCaught(data) ? 'x active' : 'x inactive'">
                                X
                            </div>
                        </td>
                    <td>
            ...
            <tbody v-else>
                    <tr v-for="data in activeMonthData.data" :key="`fish-${data.name}`" :class="checkCaught(data) ? 'content-row caught' : 'content-row'" @click="setCaught(data)">
                        <td class="caught">
                            <div :class="checkCaught(data) ? 'x active' : 'x inactive'">
                                X
                            </div>
...
export default {
    ...
    methods: {
        checkCaught(val) {
            if (this.payload.data === 'fish' || localStorage.getItem('last') === 'fish') {
        ...
        setCaught(val) {
            const last = localStorage.getItem('last') === 'fish' ? 'myfishies' : 'myinsecties';
    ...
    computed: {
        usedData() {
            return localStorage.getItem('last') === 'fish' ? this.payload.fish : this.payload.insects;
        },
        activeMonthData() {
            return this.sortedData[this.payload.activeMonth];
        },
        cleanedData() {
            let clean = JSON.parse(JSON.stringify(this.usedData).replace(/<p>|<\/p>/g, ''))
        ...
        filteredData() {
            return this.cleanedData.filter(elem => elem.name.toUpperCase().includes(this.payload.filterValue.toUpperCase()));
        },
        sortedData() {
            if (this.payload.sort === 'month') {
                return this.payload.months.map((element, i) => {
                    return {
                        month: element,
